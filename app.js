...
async function crearFactura(){const p=paciente(Number(facPaciente.value)); const total=Number(facCantidad.value||1)*Number(facPrecio.value||0); facturaPreview.innerHTML=`<div class="card"><h1>FACTURA Nº ${facNumero.value||"0001"}</h1><p><b>${p.nombre||"Paciente"}</b></p><table><tr><th>Concepto</th><th>Cantidad</th><th>Precio</th><th>Total</th></tr><tr><td>${facConcepto.value}</td><td>${facCantidad.value}</td><td>${facPrecio.value}€</td><td>${total}€</td></tr></table><h2>Total: ${total}€</h2><button class="btn" onclick="window.print()">Imprimir/PDF</button></div>`}
